...
public class RSAUtils {
    ...
    public static synchronized String[] getKeyPair(){
        ...
        String publicKeyString = encoder.encodeToString(rsaPublicKey.getEncoded());
        String privateKeyString = encoder.encodeToString(rsaPrivateKey.getEncoded());
        key[0] = publicKeyString;
        key[1] = privateKeyString;
        return key;
    }



    public static synchronized KeyObject generateKeyPairarray(){
        ...
        keyObject.publicKey = keyPair.getPublic();
        keyObject.privateKey = keyPair.getPrivate();
     ...
     * @throws InvalidKeySpecException
     */
    public static RSAPublicKey getPublicKeyFromString(String mess){
        try {
            byte[] keyBytes = decoder.decode(mess);
            X509EncodedKeySpec x509EncodedKeySpec = new X509EncodedKeySpec(keyBytes);
            return (RSAPublicKey)keyFactory.generatePublic(x509EncodedKeySpec);
        }catch (InvalidKeySpecException e) {
            LOGGER.error("getPublicKey()#" + e.getMessage(),e);
     ...
     * @throws InvalidKeySpecException
     */
    public static RSAPrivateKey getPrivateKeyFromString(String mess){
        try {
            byte[] keyBytes = decoder.decode(mess);
            PKCS8EncodedKeySpec pkcs8EncodedKeySpec = new PKCS8EncodedKeySpec(keyBytes);
            return (RSAPrivateKey)keyFactory.generatePrivate(pkcs8EncodedKeySpec);
